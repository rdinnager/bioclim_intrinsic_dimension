...
lat_mat <- bioclim_mat[ , 2]
long_mat <- bioclim_mat[ , 1]
lat_embedding <- create_angle_embedding(lat_mat)
long_embedding <- create_angle_embedding(long_mat)
world_embedding <- cbind(lat_embedding, long_embedding)
...
## clear space
rm(bioclim_mat, lat_embedding, long_embedding, world_embedding, lat_mat, long_mat)
gc()
