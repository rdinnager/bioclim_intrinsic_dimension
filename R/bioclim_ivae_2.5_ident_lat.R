...
angle_embedding <- create_angle_embedding(lat_mat)
...
train_ds <- bioclim_dataset(bioclim_mat, angle_embedding)
...
vae_mod <- nn_module("CVAE",
                 ...
                 loss_function = function(reconstruction, input, mean, log_var, prior_means, prior_logvars) {
                   ...
                   loss <- torch_mean(recon1 + recon2 + kl_posterior + kl_prior)
